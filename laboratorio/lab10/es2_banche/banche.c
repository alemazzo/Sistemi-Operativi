...
void *Deposito(void *arg)
{
	...
	{
		sleep(1);
		DBGpthread_mutex_lock(&mutex[banca], "Deposito");
		totale[banca] += 10;
		operazioni[banca]++;
		usleep(100000);
		DBGpthread_mutex_unlock(&mutex[banca], "Deposito");
	}
}
...
void *Prelievo(void *arg)
{
	...
	{
		sleep(1);
		DBGpthread_mutex_lock(&mutex[banca], "Prelievo");
		totale[banca] -= 9;
		operazioni[banca]++;
		usleep(100000);
		DBGpthread_mutex_unlock(&mutex[banca], "Prelievo");
	}
}
...
void *BancaDiItalia(void *arg)
{
	while (1)
	{
		int indiceBanca, i, numOperazioni = 0, totaleFondi = 0;

		sleep(10);
		for (i = 0; i < NUMBANCHE; i++)
		{
			DBGpthread_mutex_lock(&mutex[indiceBanca], "BancaDiItalia");
			printf("Banca %d :\n\tTotale => %d\n\tOperazioni => %d \n", i, totale[i], operazioni[i]);
			fflush(stdout);
			numOperazioni += operazioni[i];
			totaleFondi += totale[i];
			DBGpthread_mutex_unlock(&mutex[indiceBanca], "BancaDiItalia");
		}
