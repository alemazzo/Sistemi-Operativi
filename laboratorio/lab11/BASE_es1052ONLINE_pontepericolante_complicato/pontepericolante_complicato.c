...
void *Auto(void *arg)
{
	...
	while (1)
	{
		...
		while ((ponte[!indiceSenso] != indiceSenso && ponte[indiceSenso] == -1) && (biglietto[indiceSenso] < myBiglietto || (indiceSenso == INDICESENSOANTIORARIO && biglietto[INDICESENSOORARIO] <= myBiglietto) || biglietto[!indiceSenso] < myBiglietto)){
		...
		ponte[indiceSenso] = indiceSenso;
		sleep(1); /* auto attraversa il ponte */
		
		printf("auto %s arriva a metà ponte \n", Plabel);
