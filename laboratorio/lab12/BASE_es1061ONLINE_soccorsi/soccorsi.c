...
pthread_mutex_t mutexNumSoccorritori;
...
void *Soccorritore(char *Label, int ruolo)
{
	while (1)
	{
		...
		NumSoccorritoriArrivati--;
		while(NumSoccorritoriArrivati != 0) DBGpthread_cond_wait(&condFineSoccorso, &mutex, Label);
...
int main(int argc, char *argv[])
{
	...
	rc = pthread_mutex_init(&mutexNumSoccorritori, NULL);
	if (rc)
		PrintERROR_andExit(rc, "pthread_mutex_init failed");
